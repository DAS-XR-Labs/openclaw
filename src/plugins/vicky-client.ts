...
if (!process.env.VICKY_SECRET_TOKEN) {
  console.warn(
    "⚠️ [VickyClient] VICKY_SECRET_TOKEN is not set. All Anonymization/Gatekeeper calls will fail (401).",
  );
}
...
export class VickyClient {
  ...
  static async checkPermission(request: ToolRequest): Promise<PolicyDecision> {
    ...
    while (attempt <= this.maxRetries) {
      ...
      try {
        ...
        const isServerError =
          err instanceof Error && err.message.includes("Vicky API Server Error");
  ...
  static async restore(text: string, sessionKey: string): Promise<string> {
    console.log(`[VickyClient] restore called for: "${text}" with session: "${sessionKey}"`);
    const result = await this.handleTextRequest(
      text,
      sessionKey,
      "/api/deanonymize",
      "restoredText",
    );
    console.log(`[VickyClient] restore result: "${result}"`);
    return result;
  }
  ...
  public static containsPlaceholder(text: string): boolean {
    // Relaxed Regex: "Contains Placeholder" (e.g. "Call EMAIL_01 now", "EMAIL-01", "EMAIL01")
    // Uses word boundaries to avoid partial matches on normal words.
    return /\b[A-Z]+[_-]?\d+\b/.test(text);
  }

  private static async handleTextRequest(
    text: string,
    sessionKey: string,
    endpoint: string,
    resultKey: string,
