...
describe('VickyClient', () => {
    ...
    test('anonymize throws on API error (Fail-Closed)', async () => {
        ...
        await expect(VickyClient.anonymize('test', mockSessionKey)).rejects.toThrow('Vicky Service Unreachable');
    });

    test('restore calls API with correct headers', async () => {
        mockFetch.mockResolvedValueOnce({
            ok: true,
            json: async () => ({ originalText: 'Hello test@example.com' }),
        } as Response);
    ...
    test('restoreRecursive handles nested objects and arrays', async () => {
        ...
        mockFetch.mockImplementation(async (url, init) => {
            ...
            if (body.text === 'EMAIL_01') {
                return {
                    ok: true,
                    json: async () => ({ originalText: 'test@example.com' }),
                } as Response;
            }
            if (body.text === 'PHONE_01') {
                return {
                    ok: true,
                    json: async () => ({ originalText: '555-1234' }),
                } as Response;
            }
            return {
                ok: true,
                json: async () => ({ originalText: body.text }),
            } as Response;
        });
